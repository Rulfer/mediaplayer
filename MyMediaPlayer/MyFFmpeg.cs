...
namespace MyMediaPlayer
{
    internal static class MyFFmpeg
    {
        ...
        private static async Task ExtractFPS()
        {
            ...
            string singleFrameArgument = $@"-hwaccel cuda -re -ss 00:00:00 -i {VideoPath} -map 0:v -preset ultrafast -s {width}x{height} -threads {4} -vf fps={CACHED_FPS} -f image2pipe -vcodec rawvideo -pix_fmt bgr24 pipe:1";
            ...
            // Why don't I merge this function and the task below?
        ...
        private static async void ProcessAudioStream(Stream audioStream)
        {
            ...
            {
                var waveFormat = new WaveFormat(48000, 16, 2); // Match stream
